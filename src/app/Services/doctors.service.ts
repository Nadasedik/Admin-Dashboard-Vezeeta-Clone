...
export class DoctorsService {
  constructor(private _firestore: AngularFirestore, private _router: Router) { }
  ...
  getAllDoctors() {
    let allData: any[] = [];
    const allDoctors = this._firestore
      .collection('/Doctors_Collection/WOB3F9GigX8UX0O1v8zE/GeneralDoctors')
      .get()
      .toPromise();
  ...
  getSpecificDoctorByID(id: string) {
    let dataOfThisDoctor = this._firestore
      .collection('/Doctors_Collection/WOB3F9GigX8UX0O1v8zE/GeneralDoctors')
      .doc(id)
      .get()
      .toPromise();
    let specificDoctor;
    return dataOfThisDoctor
      .then((data) => {
        data?.data();
